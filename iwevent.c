 ...
 */
static inline int print_event_token(
        ...
        int has_range)
{
  ...
  switch (event->cmd) {
      ...
      case SIOCGIWESSID:
      {
          ...
          if (event->u.essid.flags) {
              ...
              if ((event->u.essid.flags & IW_ENCODE_INDEX) > 1)
                  printf("%s ESSID:\"%s\" [%d]\n", prefix, essid, (event->u.essid.flags & IW_ENCODE_INDEX));
              else
                  printf("%s ESSID:\"%s\"\n", prefix, essid);
          } else {
      ...
      case SIOCGIWTHRSPY:
      {
          ...
          if ((event->u.data.pointer) && (event->u.data.length)) {
              memcpy(&threshold, event->u.data.pointer, sizeof(struct iw_thrspy));
              printf("Spy threshold crossed on address:%s\n", iw_saether_ntop(&threshold.addr, buffer));
              iw_print_stats(buffer, sizeof(buffer), &threshold.qual, iw_range, has_range);
              printf("                            Link %s\n", buffer);
          } else {
      ...
      case IWEVMICHAELMICFAILURE:
          if (event->u.data.length >= sizeof(struct iw_michaelmicfailure)) {
              ...
              printf("Michael MIC failure flags:0x%X src_addr:%s tsc:%s\n",
                      mf.flags, iw_saether_ntop(&mf.src_addr, buffer2), iw_hexdump(buffer,
                      sizeof(buffer), mf.tsc, IW_ENCODE_SEQ_MAX_SIZE));
          }
          break;
      case IWEVASSOCREQIE:
