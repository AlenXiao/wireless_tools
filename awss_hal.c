...
static void display_info(struct wireless_info *info, char *ifname)
{
  ...
  if (info->b.has_essid) {
      ...
      /* Does it have an ESSID index ? */
          if ((info->b.essid_on & IW_ENCODE_INDEX) > 1)
              printf("ESSID:\"%s\" [%d]  ", info->b.essid, (info->b.essid_on & IW_ENCODE_INDEX));
          else
  ...
  if (info->has_sens) {
      ...
      if (info->has_range) /* Display in dBm ? */
  ...
  if (info->has_rts) {
      ...
      if (info->rts.disabled) {
          ...
          /* Fixed ? */
          printf("RTS thr%c%d B   ", info->rts.fixed ? '=' : ':', info->rts.value);
      }
      tokens += 3;
  }
  ...
  if (info->has_frag) {
      ...
      if (info->frag.disabled) {
          ...
          /* Fixed ? */
          printf("Fragment thr%c%d B   ", info->frag.fixed ? '=' : ':', info->frag.value);
      }
  }
                           ...
                           int modnum)
{
  ...
  } while ((i < count) && (k < modnum));  /* For as long as current arg matched and not out of args */
 ...
 * Set ESSID
 */
static int set_essid_info(int skfd, char *ifname,
                          ...
                          int count)     /* Args count */
{
  ...
  if ((!strcasecmp(args[0], "off")) || (!strcasecmp(args[0], "any"))) {
      ...
      if (strlen(args[i]) > IW_ESSID_MAX_SIZE) {
          ...
          if ((i < count) && (sscanf(args[i], "[%i]", &temp) == 1) && (temp > 0) && (temp < IW_ENCODE_INDEX)) {
 ...
 * Set Mode
 */
static int set_mode_info(int skfd,
                         char *ifname,
                         char *args[], /* Command line args */
                         int count)    /* Args count */
{
  struct iwreq wrq;
  unsigned int k = 0;        /* Must be unsigned */
  ...
  if (sscanf(args[0], "%i", &k) != 1) {
      k = 0;
      while ((k < IW_NUM_OPER_MODE) && strncasecmp(args[0], iw_operation_mode[k], 3))
          k ++;
  }
  printf("%s, line:%u, k:%d\r\n", __func__, __LINE__, k);
 ...
 */
static int set_freq_info(int skfd, char *ifname, char chan)    /* Args count */
{
    struct iwreq wrq;
    int i = 1;
    double freq = (double)chan;
 ...
 * Set encryption
 */
static int set_enc_info(int    skfd, char *ifname,
                        char *args[], /* Command line args */
                        int count)    /* Args count */
{
  struct iwreq wrq;
  ...
  if (!strcasecmp(args[0], "on")) {
      ...
      do {
          ...
          /* -- Check for the key -- */
          if (i < count) {
              keylen = iw_in_key_full(skfd, ifname, args[i], key, &wrq.u.data.flags);
              if (keylen > 0) {
 ...
 */
static int set_power_info(int skfd,
                          char *ifname,
                          char *args[],  /* Command line args */
                          int count)     /* Args count */
{
 ...
 * Set Nickname
 */
static int set_nick_info(int skfd, char *ifname,
                         char *args[], /* Command line args */
                         int count)    /* Args count */
{
  struct iwreq wrq;
 ...
 * Set commit
 */
static int set_nwid_info(int skfd, char *ifname,
                         char *args[],  /* Command line args */
                         int count)     /* Args count */
{
  struct iwreq wrq;
 ...
 * Set AP Address
 */
static int set_apaddr_info(int skfd, char *ifname,
                           char *args[],  /* Command line args */
                           int count)     /* Args count */
{
  struct iwreq wrq;
 ...
 * Set Tx Power
 */
static int set_txpower_info(int skfd, char *ifname,
                            char *args[],  /* Command line args */
                            int count)     /* Args count */
{
  struct iwreq wrq;
  ...
  if (!strcasecmp(args[0], "off")) {
      ...
      ismwatt = ((strchr(args[0], 'm') != NULL) && (strchr(args[0], 'd') == NULL));
 ...
 * Set Sensitivity
 */
static int set_sens_info(int skfd, char *ifname,
          char *args[],  /* Command line args */
          int count)     /* Args count */
{
  struct iwreq wrq;
 ...
 * Set Retry Limit
 */
static int set_retry_info(int skfd, char *ifname,
           char *args[],  /* Command line args */
           int count)     /* Args count */
{
  struct iwreq wrq;
 ...
 * Set RTS Threshold
 */
static int set_rts_info(int    skfd, char *ifname,
                        char *args[],  /* Command line args */
                        int count)     /* Args count */
{
  struct iwreq wrq;
 ...
 */
static int set_frag_info(int skfd, char *ifname,
          char *args[],  /* Command line args */
          int count)     /* Args count */
{
  struct iwreq wrq;
 ...
 * Set Modulation
 */
static int set_modulation_info(int skfd, char *ifname,
                               char *args[],  /* Command line args */
                               int    count)    /* Args count */
{
  struct iwreq wrq;
  ...
  if (!strcasecmp(args[0], "auto")) {
      ...
      } while ((i < count) && (k < IW_SIZE_MODUL_LIST)); /* For as long as current arg matched and not out of args */
 ...
 * Set commit
 */
static int set_commit_info(int skfd, char *ifname,
                           char *args[],  /* Command line args */
                           int count)     /* Args count */
{
  struct iwreq wrq;
...
int awss_socket_close(int fd)
{
    return close(fd);
}

void HAL_Awss_Switch_Channel(char primary_chan, char second_chan, uint8_t bssid[6])
{
    int skfd;  /* generic raw socket desc. */

    /* Create a channel to the NET kernel. */
    if ((skfd = awss_sockets_open()) < 0) {
        perror("socket");
        exit(-1);
    }

    set_freq_info(skfd, AWSS_IFACE_NAME, primary_chan);

    awss_socket_close(skfd);
}
...
int HAL_Awss_Connect_Ap(uint32_t connnection_timeout_ms, char ssid[32]
                        char passwd[64], enum AWSS_AUTH_TYPE auth,
                        enum AWSS_ENC_TYPE encry, uint8_t bssid[6],
                        uint8_t channel)
{
}
